...
| Figma MCP POST blocked / request fails when SVG or text contains shell strings | Cloudflare WAF flags payloads containing shell/curl/command tokens (pipes, `ssh`, `curl -H`, destructive command names). | Use generic filler for decorative pseudo-code text; avoid embedding real shell commands in node text sent through the MCP. |
| Text touches / overflows its box edges (e.g. a label in a pill) | Fixed-width box with a centered label that is wider than you assumed. Centering hides it until the longest item clips. | Size the box from the text: measure `t.width`/`t.height` after setting `characters`, then `resize(t.width + 2*padX, t.height + 2*padY)`. For fixed boxes, check the longest label fits with margin. Min ~14-16px horizontal padding. See SKILL core principle 5. |
...
| `use_figma` returns "Code executed with no return value" even with a trailing `JSON.stringify(...)` | The tool does not reliably surface return values. | Don't depend on the return. Name every node on creation, then call `get_metadata` on the parent frame and find the node by name to get its id (needed for `upload_assets`, `get_screenshot`, next-call edits). |
| Can't crop/resize a source image before importing; ImageMagick missing (on Windows `convert` is the disk tool) | The sandbox has no filesystem, so preprocessing happens outside Figma. | Crop/resize with Python + Pillow on an interpreter that actually has it (a minimal/bundled Python may not); probe `import PIL` first. Then import via `upload_assets`. `09-raster-images.md`. |
| Imported image is distorted or letterboxed | Holder rect aspect ≠ image aspect, or wrong `scaleMode`. | Size the holder to the image aspect (`holderH = holderW / (imgW/imgH)`) and use `FILL`; use `FIT` for transparent logos to preserve aspect. `09-raster-images.md`. |
